...
class LogoProcedure:
    ...
    @classmethod
    def make_procedure(
        cls, name, required_inputs, optional_inputs, rest_input, default_arity, tokens
    ):
        p = cls()
        p.name = name
    ...
    def __str__(self):
        ...
        for name, value in self.optional_inputs:
            if _is_list(value):
                valuestr = _list_contents_repr(
                    value, include_braces=True, escape_delimiters=True
                )
            else:
                valuestr = str(value)
...
def create_primitives_map():
    ...
    m["arctan"] = make_primitive(
        "arctan", ["x"], [], "y", 1, process_arctan, max_arity=2
    )
    m["back"] = make_primitive("back", ["dist"], [], None, 1, process_back)
    m["bk"] = m["back"]
    m["background"] = make_primitive(
        "background", ["color"], [], None, 0, process_background
    )
    m["bg"] = m["background"]
    m["beforep"] = make_primitive(
        "beforep", ["word1", "word2"], [], None, 2, process_beforep
    )
    m["before?"] = m["beforep"]
    m["butfirst"] = make_primitive(
        "butfirst", ["wordlist"], [], None, 1, process_butfirst
    )
    m["bf"] = m["butfirst"]
    m["butfirsts"] = make_primitive(
        "butfirsts", ["list"], [], None, 1, process_butfirsts
    )
    m["bfs"] = m["butfirsts"]
    m["butlast"] = make_primitive("butlast", ["wordlist"], [], None, 1, process_butlast)
    m["bl"] = m["butlast"]
    m["cartesian.heading"] = make_primitive(
        "cartesian.heading", ["heading"], [], None, 1, process_cartesian_heading
    )
    m["cascade"] = make_primitive(
        "cacade", ["endtest", "template", "startvalue"], [], "args", 3, process_cascade
    )
    ...
    m["clearscreen"] = make_primitive(
        "clearscreen", [], [], None, 0, process_clearscreen
    )
    m["cs"] = m["clearscreen"]
    m["combine"] = make_primitive(
        "combine", ["thing1", "thing2"], [], None, 2, process_combine
    )
    ...
    m["dequeue"] = make_primitive(
        "dequeue", ["queuename"], [], None, 1, process_dequeue
    )
    m["difference"] = make_primitive(
        "difference", ["num1", "num2"], [], None, 2, process_difference
    )
    m["do.until"] = make_primitive(
        "do.until", ["instrlist", "tfexpr"], [], None, 2, process_dountil
    )
    m["do.while"] = make_primitive(
        "do.while", ["instrlist", "tfexpr"], [], None, 2, process_dowhile
    )
    m["emptyp"] = make_primitive("emptyp", ["thing"], [], None, 1, process_emptyp)
    m["empty?"] = m["emptyp"]
    m["equalp"] = make_primitive(
        "equalp", ["thing1", "thing2"], [], None, 2, process_equalp
    )
    m["equal?"] = m["equalp"]
    m["exp"] = make_primitive("exp", ["num"], [], None, 1, process_exp)
    m["ext.ellipse"] = make_primitive(
        "ext.ellipse",
        ["major", "minor"],
        [("angle", 360), ("clockwise", "true")],
        None,
        2,
        process_ext_ellipse,
    )
    m["ext.unfilled"] = make_primitive(
        "ext.unfilled", ["instructions"], [], "args", 1, process_ext_unfilled
    )
    m["filter"] = make_primitive(
        "filter", ["tftemplate", "data"], [], "args", 2, process_filter
    )
    m["filled"] = make_primitive(
        "filled", ["color", "instructions"], [], "args", 2, process_filled
    )
    m["find"] = make_primitive(
        "find", ["tftemplate", "data"], [], "args", 2, process_find
    )
    ...
    m["for"] = make_primitive(
        "for", ["forcontrol", "instrlist"], [], None, 2, process_for
    )
    m["foreach"] = make_primitive(
        "foreach", ["data", "template"], [], "args", 2, process_foreach
    )
    m["forward"] = make_primitive("forward", ["dist"], [], None, 1, process_forward)
    m["fd"] = m["forward"]
    m["fput"] = make_primitive("fput", ["thing", "list"], [], None, 2, process_fput)
    m["greaterequalp"] = make_primitive(
        "greaterequalp", ["num1", "num2"], [], None, 2, process_greaterequalp
    )
    m["greaterequal?"] = m["greaterequalp"]
    m["greaterp"] = make_primitive(
        "greaterp", ["num1", "num2"], [], None, 2, process_greaterp
    )
    ...
    m["ifelse"] = make_primitive(
        "ifelse", ["tf", "instrlist1", "instrlist2"], [], None, 3, process_ifelse
    )
    ...
    m["label"] = make_primitive(
        "label",
        ["text"],
        [("move", "false"), ("align", "left"), ("font", ["Arial", 8, "normal"])],
        None,
        1,
        process_label,
    )
    ...
    m["lt"] = m["left"]
    m["lessequalp"] = make_primitive(
        "lessequalp", ["num1", "num2"], [], None, 2, process_lessequalp
    )
    m["lessequal?"] = m["lessequalp"]
    m["lessp"] = make_primitive("lessp", ["num1", "num2"], [], None, 2, process_lessp)
    m["less?"] = m["lessp"]
    m["list"] = make_primitive(
        "list", ["thing1", "thing2"], [], "others", 2, process_list
    )
    ...
    m["localmake"] = make_primitive(
        "localmake", ["varname", "value"], [], None, 2, process_localmake
    )
    m["lput"] = make_primitive("lput", ["thing", "list"], [], None, 2, process_lput)
    m["log10"] = make_primitive("log10", ["num"], [], None, 1, process_log10)
    m["lowercase"] = make_primitive(
        "lowercase", ["word"], [], None, 1, process_lowercase
    )
    ...
    m["map.se"] = make_primitive(
        "map.se", ["template", "data"], [], "args", 2, process_map_se
    )
    m["member"] = make_primitive(
        "member", ["thing1", "thing2"], [], None, 2, process_member
    )
    m["memberp"] = make_primitive(
        "memberp", ["thing1", "thing2"], [], None, 2, process_memberp
    )
    m["member?"] = m["memberp"]
    m["modulo"] = make_primitive(
        "modulo", ["num1", "num2"], [], None, 2, process_modulo
    )
    m["not"] = make_primitive("not", ["tf"], [], None, 1, process_not)
    m["notequalp"] = make_primitive(
        "notequalp", ["thing1", "thing2"], [], None, 2, process_notequalp
    )
    ...
    m["printout"] = make_primitive(
        "printout", ["contentslist"], [], None, 1, process_printout
    )
    m["po"] = m["printout"]
    ...
    m["polygon"] = make_primitive(
        "polygon",
        ["n", "radius"],
        [("clockwise", "true"), ("sides", ":n")],
        None,
        2,
        process_polygon,
    )
    ...
    m["pr"] = m["print"]
    m["product"] = make_primitive(
        "product", ["num1", "num2"], [], "nums", 2, process_product
    )
    m["push"] = make_primitive(
        "push", ["stackname", "thing"], [], None, 2, process_push
    )
    m["queue"] = make_primitive(
        "queue", ["queuename", "thing"], [], None, 2, process_queue
    )
    m["quoted"] = make_primitive("quoted", ["thing"], [], None, 1, process_quoted)
    m["quotient"] = make_primitive(
        "quotient", ["num1", "num2"], [], None, 2, process_quotient
    )
    m["radarctan"] = make_primitive(
        "radarctan", ["x"], [], "y", 1, process_radarctan, max_arity=2
    )
    m["radcos"] = make_primitive("radcos", ["radians"], [], None, 1, process_radcos)
    m["radsin"] = make_primitive("radsin", ["radians"], [], None, 1, process_radsin)
    m["random"] = make_primitive(
        "random", ["start"], [], "end", 1, process_random, max_arity=2
    )
    m["readlist"] = make_primitive("readlist", [], [], None, 0, process_readlist)
    m["remainder"] = make_primitive(
        "remainder", ["num1", "num2"], [], None, 2, process_remainder
    )
    m["rl"] = m["readlist"]
    m["reduce"] = make_primitive(
        "reduce", ["template", "data"], [], "args", 2, process_reduce
    )
    m["remove"] = make_primitive(
        "remove", ["thing", "list"], [], None, 2, process_remove
    )
    m["remdup"] = make_primitive("remdup", ["list"], [], None, 1, process_remdup)
    m["repcount"] = make_primitive("repcount", [], [], None, 0, process_repcount)
    m["#"] = m["repcount"]
    m["repeat"] = make_primitive(
        "repeat", ["num", "instructionlist"], [], None, 2, process_repeat
    )
    ...
    m["rseq"] = make_primitive(
        "rseq", ["from", "to", "count"], [], None, 3, process_rseq
    )
    m["run"] = make_primitive("run", ["instructionlist"], [], None, 1, process_run)
    m["runresult"] = make_primitive(
        "runresult", ["instructionlist"], [], None, 1, process_runresult
    )
    m["save"] = make_primitive("save", ["filename"], [], None, 1, process_save)
    m["sentence"] = make_primitive(
        "sentence", ["thing"], [], "others", 2, process_sentence
    )
    m["se"] = m["sentence"]
    m["setbackground"] = make_primitive(
        "setbackground", ["color"], [], None, 1, process_setbackground
    )
    m["setbg"] = m["setbackground"]
    m["setheading"] = make_primitive(
        "setheading", ["angle"], [], None, 1, process_setheading
    )
    m["seth"] = m["setheading"]
    m["setpencolor"] = make_primitive(
        "setpencolor", ["color"], [], None, 1, process_setpencolor
    )
    m["setpc"] = m["setpencolor"]
    m["setpensize"] = make_primitive(
        "setpensize", ["width"], [], None, 1, process_setpensize
    )
    m["setpos"] = make_primitive("setpos", ["pos"], [], None, 1, process_setpos)
    m["setspeed"] = make_primitive("setspeed", ["num"], [], None, 1, process_setspeed)
    m["setundobuffer"] = make_primitive(
        "setundobuffer", ["num"], [], None, 1, process_setundobuffer
    )
    ...
    m["substringp"] = make_primitive(
        "substringp", ["thing1", "thing2"], [], None, 2, process_substringp
    )
    ...
    m["turtle.heading"] = make_primitive(
        "turtle.heading", ["cartesian.heading"], [], None, 1, process_turtle_heading
    )
    ...
    m["undobufferentries"] = make_primitive(
        "undobufferentries", [], [], None, 0, process_undobufferentries
    )
    m["until"] = make_primitive(
        "until", ["tfexpr", "instrlist"], [], None, 2, process_until
    )
    m["uppercase"] = make_primitive(
        "uppercase", ["word"], [], None, 1, process_uppercase
    )
    m["wait"] = make_primitive("wait", ["time"], [], None, 1, process_wait)
    m["while"] = make_primitive(
        "while", ["tfexpr", "instrlist"], [], None, 2, process_while
    )
    ...
    for arg in args:
        if not arg.lower() in truth_map:
            raise errors.LogoError(
                "AND expects true/false values but received `{}` instead.".format(arg)
            )
...
def process_arctan(logo, *args):
    ...
    if len(args) == 1:
        ...
        try:
            ...
            raise errors.LogoError(
                "ARCTAN expected a number, but got `{}` instead.".format(x)
            )
    elif len(args) == 2:
        x = args[0]
        y = args[1]
...
def process_beforep(logo, word1, word2):
    ...
    for dtype in (dtype1, dtype2):
        if dtype != "word":
            raise errors.LogoError(
                "BEFOREP expects a word but got {} instead.".format(dtype)
            )
...
def process_cascade(logo, endtest, *args):
    ...
    if dtype == "word":
        try:
            ...
            raise errors.LogoError(
                "CASCADE expected an integer for its "
                "end test, but received `{}` instead.".format(endtest)
            )

        def test_end_func(count, script):
        ...
        def test_end_func(count, script):
            return _is_true(logo.process_instructionlist(script))

    else:
        raise errors.LogoError(
            "CASCADE expected an integer or template for "
            "`endtest`, but received `{}` instead.".format(endtest)
        )
...
def process_case(logo, value, clauses):
    ...
    if not _is_list(clauses):
        raise errors.LogoError(
            "CASE expected a list of clauses but received `{}` instead.".format(clauses)
        )
    for clause in clauses:
        if (not _is_list(clause)) or len(clause) != 2:
            raise errors.LogoError(
                "CASE expects a clause to be a "
                "2-member list but received `{}` instead.".format(clause)
            )
        values = clause[0]
        matched = (_is_word(values) and values.upper() == "ELSE") or (
...
def process_char(logo, codepoint):
    ...
    try:
        ...
        raise errors.LogoError(
            "CHAR expects an integer, but got `{}` instead.".format(codepoint)
        )
...
def process_cond(logo, clauses):
    ...
    if not _is_list(clauses):
        raise errors.LogoError(
            "COND expected a list of clauses but received `{}` instead.".format(clauses)
        )
    for clause in clauses:
        if (not _is_list(clause)) or len(clause) < 2:
            raise errors.LogoError(
                "COND expects a clause to be a list with at least "
                "2 members but received `{}` instead.".format(clause)
            )
        cond = clause[0]
        matched = (_is_word(cond) and cond.lower() == "else") or _is_true(
            _process_run_like("COND", logo, cond)
        )
        if matched:
            instrlist = clause[1:]
...
def process_cos(logo, degrees):
    ...
    try:
        ...
        raise errors.LogoError(
            "COS expected a number in degrees, but got `{}` instead.".format(degrees)
        )
...
def process_dec_str(logo, num):
    ...
    if _is_word(num):
        ...
        raise errors.LogoError(
            "DEC.STR expects a word argument but received `{}` instead.".format(num)
        )
...
def process_dequeue(logo, queuename):
    ...
    try:
        ...
        raise errors.LogoError(
            "Tried to DEQUEUE from `{}`, but is not a list.".format(queuename)
        )
    except IndexError:
        raise errors.LogoError(
            "Tried to DEQUEUE from an empty list, `{}`.".format(queuename)
        )
...
def process_difference(logo, num1, num2):
    ...
    for arg in (num1, num2):
        if not isinstance(arg, numbers.Number):
            raise errors.LogoError(
                "DIFFERENCE expected a number but got `{}` instead.".format(arg)
            )
    return num1 - num2
...
def process_exp(logo, num):
    ...
    try:
        ...
        raise errors.LogoError(
            "EXP expected a number, but received `{}` instead.".format(num)
        )
...
def process_ext_ellipse(logo, major, minor, angle=360, clockwise="true"):
    ...
    if not _is_number(major):
        raise errors.LogoError(
            "EXT.ELLIPSE expects `major` to be a number, but received `{}`.".format(
                major
            )
        )
    if not _is_number(minor):
        raise errors.LogoError(
            "EXT.ELLIPSE expects `minor` to be a number, but received `{}`.".format(
                minor
            )
        )
    if not _is_number(angle):
        raise errors.LogoError(
            "EXT.ELLIPSE expects `angle` to be a number, but received `{}`.".format(
                angle
            )
        )
...
def _extract_define_inputs_from_list(lst):
    ...
    while len(lst) > 0:
        ...
        if _is_list(item) and len(item) == 2:
            ...
            if not _is_word(varname):
                raise errors.LogoError(
                    "Expected an optional parameter name, but received `{}` instead.".format(
                        varname
                    )
                )
    ...
    if len(lst) > 0:
        ...
        if _is_word(varname):
            ...
            raise errors.LogoError(
                "Expected a rest input parameter name, but received `{}` instead.".format(
                    varname
                )
            )
...
def _create_template(cmd, logo, data_lists, template):
    ...
    * `procedure-text` - A Procedure object.
    ...
    if not len(set([len(x) for x in data_lists])) == 1:
        raise errors.LogoError(
            "{} expects all data lists to be of equal size.".format(cmd)
        )
    ...
    if _is_list(template):
        ...
        if is_proc_text_form:
            (
                ...
                optional_inputs,
                rest_input,
            ) = _extract_define_inputs_from_list(first_item)
            ...
            if data_list_count != named_slot_count:
                raise errors.LogoError(
                    (
                        "{} received {} data lists, but its "
                        "template contains {} named slots."
                    ).format(cmd, data_list_count, named_slot_count)
                )
        ...
        if proc is None:
            raise errors.LogoError(
                "{} received procedure name, `{}`, but I don't know how to `{}`.".format(
        ...
        is_valid_arity = (arity <= max_arity or max_arity == -1) and (
            arity >= min_arity
        )
        if is_valid_arity:
            ...
            raise errors.LogoError(
                (
                    "FOREACH received {} data lists, but named "
                    "procedure `{}` takes at least {} arguments."
                ).format(arity, template, min_arity)
            )
        else:
            raise errors.LogoError(
                (
                    "FOREACH received {} data lists, but "
                    "named procedure `{}` takes at most {} arguments."
                ).format(arity, template, max_arity)
            )
...
def process_foreach(logo, *args):
    ...
    if len(args) < 2:
        raise errors.LogoError(
            "FOREACH expects at least 2 arguments, but received `{}` instead.".format(
                args
            )
        )
    template = args[-1]
...
def process_for(logo, forcontrol, instrlist):
    ...
    for arg in (forcontrol, instrlist):
        ...
        if dtype != "list":
            raise errors.LogoError(
                "FOR expects a list but received `{}` instead.".format(arg)
            )
    if len(forcontrol) not in (3, 4):
        raise errors.LogoError(
            (
                "FOR expects a control list with 3 or 4 "
                "members, but received `{}` instead."
            ).format(forcontrol)
        )
...
def process_greaterequalp(logo, num1, num2):
    ...
    for arg in (num1, num2):
        if not isinstance(arg, numbers.Number):
            raise errors.LogoError(
                "GREATEREQUALP expects numbers, but received `{}` instead.".format(arg)
            )
    if num1 >= num2:
        return "true"
    else:
...
def process_greaterp(logo, num1, num2):
    ...
    for arg in (num1, num2):
        if not isinstance(arg, numbers.Number):
            raise errors.LogoError(
                "GREATERP expects numbers, but received `{}` instead.".format(arg)
            )
    if num1 > num2:
        return "true"
    else:
...
def process_if(logo, tf, instrlist, instrlist2=None):
    ...
    try:
        ...
        raise errors.LogoError(
            "IF expects TRUE/FALSE but received `{}` instead.".format(tf)
        )
    ...
    if dtype != "list":
        raise errors.LogoError(
            "IF expects instructionlist but received `{}` instead.".format(instrlist)
        )
    if instrlist2 is not None:
        if dtype != "list":
            raise errors.LogoError(
                "IF expects instructionlist but received `{}` instead.".format(
                    instrlist2
                )
            )
    if tf == "true":
...
def process_ifelse(logo, tf, instrlist1, instrlist2):
    ...
    try:
        ...
        raise errors.LogoError(
            "IFELSE expects TRUE/FALSE but received `{}` instead.".format(tf)
        )
    if tf not in ("true", "false"):
        raise errors.LogoError(
            "IFELSE expects TRUE/FALSE but received `{}` instead.".format(tf)
...
def process_iseq(logo, frm, to):
    ...
    try:
        ...
        raise errors.LogoError(
            "ISEQ expects numbers, but received `{}`, `{}` instead.".format(frm, to)
        )
...
def process_label(logo, text, move="false", align="left", font=["Arial", 8, "normal"]):
    ...
    if not _is_word(text):
        raise errors.LogoError(
            (
                "LABEL expects a word for its `text` argument, "
                "but it received `{}` instead."
            ).format(text)
        )
    if not _is_word(align):
        raise errors.LogoError(
            "LABEL expects `left`, `right`, or `center` "
            "for its `align` argument, but it received `{}` instead.".format(align)
        )
    if align not in ("left", "right", "center"):
        raise errors.LogoError(
            "LABEL expects `left`, `right`, or `center` for its "
            "`align` argument, but it received `{}` instead.".format(align)
        )
    if not _is_list(font):
        raise errors.LogoError(
            "LABEL expects a list for its `font` argument, "
            "but it received `{}` instead.".format(font)
        )
    font = tuple(font)
    if len(font) != 3:
        raise errors.LogoError(
            "LABEL expects a 3-word list for its `font` "
            "argument, but it received `{}` instead.".format(font)
        )
    turtle = logo.turtle
    try:
        turtle.write(text, move=move, align=align, font=font)
    except errors.LogoError:
        raise
    except Exception:
        tb_string = traceback.format_exc()
        raise errors.LogoError(
            "The turtle encountered an error while running the LABEL command:\n{}".format(
                tb_string
            )
        )
...
def process_lessequalp(logo, num1, num2):
    ...
    for arg in (num1, num2):
        if not isinstance(arg, numbers.Number):
            raise errors.LogoError(
                "LESSEQUALP expects numbers, but received `{}` instead.".format(arg)
            )
    if num1 <= num2:
        return "true"
    else:
...
def process_lessp(logo, num1, num2):
    ...
    for arg in (num1, num2):
        if not isinstance(arg, numbers.Number):
            raise errors.LogoError(
                "LESSP expects numbers, but received `{}` instead.".format(arg)
            )
    if num1 < num2:
        return "true"
    else:
    ...
    if len(args) == 1:
        ...
        if dtype == "word":
            ...
            for varname in arg:
                ...
                if dtype2 != "word":
                    raise errors.LogoError(
                        "LOCAL expects a list of words, but received `{}` instead.".format(
                            varname
                        )
                    )
                scope[varname] = None
            ...
            raise errors.LogoError(
                "LOCAL expects a word or a list or words, but received `{}` instead.".format(
                    arg
                )
            )
    else:
...
def process_log10(logo, num):
    ...
    try:
        ...
        raise errors.LogoError(
            "LOG10 expected a number, but received `{}` instead.".format(num)
        )
...
def process_lowercase(logo, word):
    ...
    try:
        ...
        raise errors.LogoError(
            "LOWERCASE expected a word but got a {} instead.".format(
                _datatypename(word)
            )
        )
...
def process_ln(logo, num):
    ...
    try:
        ...
        raise errors.LogoError(
            "LN expected a number, but received `{}` instead.".format(num)
        )
...
def process_member(logo, thing1, thing2):
    ...
    if dtype1 == dtype2 == "word":
        ...
        raise errors.LogoError(
            "MEMBER expects a word or list for thing2 but got a {}.".format(dtype2)
        )
...
def process_modulo(logo, num1, num2):
    ...
    for arg in (num1, num2):
        if not isinstance(arg, numbers.Number):
            raise errors.LogoError(
                "MODULO expected a number but got `{}` instead.".format(arg)
            )
...
def process_not(logo, tf):
    ...
    if _is_true(tf):
        ...
        raise errors.LogoError(
            "AND expects true/false values but received `{}` instead.".format(tf)
        )
...
def process_or(logo, *args):
    ...
    for arg in args:
        if not arg.lower() in truth_map:
            raise errors.LogoError(
                "OR expects true/false values but received `{}` instead.".format(arg)
            )
...
def process_printout(logo, contentslist):
    ...
    if not _is_list(contentslist):
        raise errors.LogoError(
            "PRINTOUT expected a list of words, but received `{}` instead.".format(
                contentslist
            )
        )
...
def process_polygon(logo, n, radius, clockwise="true", sides=None):
    ...
    try:
        ...
        raise errors.LogoError(
            "POLYGON expects an integer, `n`, but received `{}` instead.".format(n)
        )
    try:
        radius = float(radius)
    except (TypeError, ValueError):
        raise errors.LogoError(
            "POLYGON expects a number for `radius`, but received `{}` instead.".format(
                n
            )
        )
    if sides is None:
...
def process_pop(logo, stackname):
    ...
    try:
        ...
        raise errors.LogoError(
            "Tried to POP from `{}`, but it is not a list.".format(stackname)
        )
    except IndexError:
...
def process_power(logo, num1, num2):
    ...
    try:
        ...
        raise errors.LogoError(
            "POWER expected a number, but received [{}, {}] instead.".format(num1, num2)
        )
...
def process_print(logo, *args):
    ...
    for arg in args:
        if _datatypename(arg) == "list":
            reps.append(
                _list_contents_repr(arg, include_braces=False, escape_delimiters=False)
            )
...
def process_product(logo, *args):
    ...
    for arg in args:
        if not isinstance(arg, numbers.Number):
            raise errors.LogoError(
                "PRODUCT expected a number but got `{}` instead.".format(arg)
            )
...
def process_push(logo, stackname, thing):
    ...
    try:
        ...
        raise errors.LogoError(
            "Tried to PUSH to `{}`, but is not a list.".format(stackname)
        )
...
def process_queue(logo, queuename, thing):
    ...
    try:
        ...
        raise errors.LogoError(
            "Tried to QUEUE to `{}`, but it is not a list.".format(queuename)
        )
...
def process_quotient(logo, num1, num2):
    ...
    for arg in (num1, num2):
        if not isinstance(arg, numbers.Number):
            raise errors.LogoError(
                "QUOTIENT expected a number but got `{}` instead.".format(arg)
            )
    return num1 / num2
...
def process_radarctan(logo, *args):
    ...
    if len(args) == 1:
        ...
        try:
            ...
            raise errors.LogoError(
                "RADARCTAN expected a number, but got `{}` instead.".format(x)
            )
    elif len(args) == 2:
        x = args[0]
        y = args[1]
...
def process_radcos(logo, radians):
    ...
    try:
        ...
        raise errors.LogoError(
            "RADCOS expected a number in radians, but got `{}` instead.".format(radians)
        )
...
def process_radsin(logo, radians):
    ...
    try:
        ...
        raise errors.LogoError(
            "RADSIN expected a number in radians, but got `{}` instead.".format(radians)
        )
...
def process_random(logo, *args):
    ...
    if len(args) == 1:
        try:
            ...
            raise errors.LogoError(
                "RANDOM expects a non-negative integer, but received `{}` instead.".format(
                    args[0]
                )
            )
    else:
        try:
            ...
            raise errors.LogoError(
                "RANDOM expects integers `start` <= `end` "
                "but received `{}`, `{}` instead.".format(args[0], args[1])
            )
...
def process_remainder(logo, num1, num2):
    ...
    for arg in (num1, num2):
        if not isinstance(arg, numbers.Number):
            raise errors.LogoError(
                "REMAINDER expected a number but got `{}` instead.".format(arg)
            )
    absval = abs(num1 % num2)
    if num1 < 0:
        return -absval
...
def process_remove(logo, thing, lst):
    ...
    if _datatypename(lst) == "list":
        ...
        raise errors.LogoError(
            "REMOVE cannot be used on a {}.".format(_datatypename(lst))
        )
...
def process_repeat(logo, num, instructionlist):
    ...
    if not isinstance(num, numbers.Number):
        raise errors.LogoError(
            "REPEAT expects a number and an "
            "instructionlist, but received `{}` instead.".format(num)
        )
    dtype = _datatypename(instructionlist)
    if dtype != "list":
        raise errors.LogoError(
            "REPEAT expects a number and an "
            "instructionlist, but received `{}` instead.".format(instructionlist)
        )
    logo.create_repcount_scope()
    try:
        if int(num) == num:
            ...
            raise errors.LogoError(
                "REPEAT expects an integer, but recieved `{}` instead.".format(num)
            )
        for i in range(num):
...
def process_round(logo, num):
    ...
    try:
        ...
        raise errors.LogoError(
            "ROUND expects a number, but received `{}` instead.".format(num)
        )
...
def process_rseq(logo, frm, to, count):
    ...
    try:
        ...
        raise errors.LogoError(
            "RSEQ expected numbers, but got `{}`, `{}`, `{}` instead.".format(
                frm, to, count
            )
        )
...
def _process_run_like(cmd, logo, instructionlist):
    ...
    if dtype == "list":
        ...
        raise errors.LogoError(
            "{} expects a word or list, but received `{}` instead.".format(
                cmd, instructionlist
            )
        )
...
def process_setpos(logo, pos):
    ...
    if not _is_list(pos):
        raise errors.LogoError(
            "SETPOS expected a list but received `{}` instead.".format(pos)
        )
    if len(pos) != 2:
        raise errors.LogoError(
            "SETPOS expected a list with 2 members but received `{}` instead.".format(
                pos
            )
        )
...
def process_show(logo, *args):
    ...
    for arg in args:
        ...
        if dtype == "list":
            ...
            raise errors.LogoError(
                "SHOW doesn't know how to show type {}.".format(dtype)
            )
    print(" ".join(reps), file=logo.stdout)
...
def process_sin(logo, degrees):
    ...
    try:
        ...
        raise errors.LogoError(
            "SIN expected a number in degrees, but got `{}` instead.".format(degrees)
        )
...
def process_sqrt(logo, num):
    ...
    try:
        ...
        raise errors.LogoError(
            "SQRT expects a number, but received `{}` instead.".format(num)
        )
    except ValueError:
        raise errors.LogoError(
            "SQRT expects a non-negative number, but received `{}` instead.".format(num)
        )
...
def process_substringp(logo, thing1, thing2):
    ...
    for thing in (thing1, thing2):
        ...
        if dtype != "word":
            raise errors.LogoError(
                "SUBSTRINGP expects a word, but got a {} instead.".format(dtype)
            )
...
def process_sum(logo, *args):
    ...
    for arg in args:
        if not _is_number(arg):
            raise errors.LogoError(
                "SUM expected a number but got `{}` instead.".format(arg)
            )
    return sum(args)
...
def process_to(logo, tokens):
    ...
    try:
        ...
        try:
            ...
            raise errors.ExpectedEndError(
                "Expected END to complete procedure `{}`.".format(procedure_name)
            )

        def _test(x):
            return hasattr(x, "lower") and x.lower() == "end"
...
def process_unicode(logo, char):
    ...
    try:
        ...
        raise errors.LogoError(
            "UNICODE expects a single character, but got `{}`.".format(char)
        )
...
def process_uppercase(logo, word):
    ...
    try:
        ...
        raise errors.LogoError(
            "UPPERCASE expected a word but got a {} instead.".format(
                _datatypename(word)
            )
        )
...
def process_word(logo, *args):
    ...
    for arg in args:
        ...
        if dtype != "word":
            raise errors.LogoError(
                "Expected a word, but got a {} instead.".format(dtype)
            )
    return "".join(args)
...
def _list_contents_repr(o, include_braces=True, escape_delimiters=True):
    ...
    if dtype == "list":
        rep = " ".join(
            [_list_contents_repr(x, escape_delimiters=escape_delimiters) for x in o]
        )
        if include_braces:
            rep = "[{}]".format(rep)
        return rep
...
def _get_logo_repr(tokens, line_break=70):
    ...
    while len(tokens) > 0:
        ...
        if _is_list(token):
            ...
            parts.append(
                "({})".format(_list_contents_repr(list(token), include_braces=False))
            )
        else:
...
def _get_color(cmdname, color):
    ...
    if _is_list(color):
        ...
        for c in color:
            ...
            if cint != c:
                raise errors.LogoError(
                    "{} expects a list of integers, but received `{}` instead.".format(
                        cmdname, color
                    )
                )
            temp.append(cint)
