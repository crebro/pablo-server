...
class NRF24:
    ...
    def __init__(self, gpio, spidev):
        ...
        self.dynamic_payloads_enabled = (
            False  # *< Whether dynamic payloads are enabled.
        )
    ...
    def print_observe_tx(self, value):
        print(
            ...
            % (value, value >> NRF24.PLOS_CNT, value & 15)
        )
    ...
    def begin(
        ...
        self.write_register(
            NRF24.STATUS, _BV(NRF24.RX_DR) | _BV(NRF24.TX_DS) | _BV(NRF24.MAX_RT)
        )
    ...
    def powerDown(self):
        self.write_register(
            NRF24.CONFIG, self.read_register(NRF24.CONFIG) & ~_BV(NRF24.PWR_UP)
        )
    ...
    def enableDynamicPayloads(self):
        ...
        self.write_register(
            NRF24.FEATURE, self.read_register(NRF24.FEATURE) | _BV(NRF24.EN_DPL)
        )
        ...
        self.write_register(
            NRF24.DYNPD,
            self.read_register(NRF24.DYNPD)
            | _BV(NRF24.DPL_P5)
            | _BV(NRF24.DPL_P4)
            | _BV(NRF24.DPL_P3)
            | _BV(NRF24.DPL_P2)
    ...
    def enableAckPayload(self):
        ...
        self.write_register(
            NRF24.FEATURE,
            self.read_register(NRF24.FEATURE)
            | _BV(NRF24.EN_ACK_PAY)
            | _BV(NRF24.EN_DPL),
        )
        ...
        self.write_register(
            NRF24.DYNPD,
            self.read_register(NRF24.DYNPD) | _BV(NRF24.DPL_P1) | _BV(NRF24.DPL_P0),
        )
    ...
    def getPALevel(self):
        power = self.read_register(NRF24.RF_SETUP) & (
    ...
    def getDataRate(self):
        dr = self.read_register(NRF24.RF_SETUP) & (
    ...
    def setCRCLength(self, length):
        config = self.read_register(NRF24.CONFIG) & ~(
            _BV(NRF24.CRC_16) | _BV(NRF24.CRC_ENABLED)
        )
    ...
    def getCRCLength(self):
        ...
        config = self.read_register(NRF24.CONFIG) & (
            _BV(NRF24.CRCO) | _BV(NRF24.EN_CRC)
        )
    ...
    def setRetries(self, delay, count):
        ...
        self.write_register(
            NRF24.SETUP_RETR, (delay & 0xF) << NRF24.ARD | (count & 0xF)
        )
    ...
    def getMaxTimeout(self):  # seconds
        ...
        tout = (
            ((250 + (250 * ((retries & 0xF0) >> 4))) * (retries & 0x0F)) / 1000000.0 * 2
        ) + 0.008
